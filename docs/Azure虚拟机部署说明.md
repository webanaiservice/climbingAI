...
需要云端模型时，把 `DEROUTER_API_KEY` 仅写入服务器私密配置。生产 Compose 只把它传给 API 容器，Web 不接收该密钥；未设置密钥时仍使用本地求解回退。`infra/configure-route-setting-production.sh` 可从标准输入接收密钥并原子更新生产环境文件，不在命令参数或输出中显示密钥。不要把密钥写入发布包、Git、浏览器变量或命令输出。灰度验证应检查三种模式、定线生成与回退、原有业务模块以及 Worker 心跳。
